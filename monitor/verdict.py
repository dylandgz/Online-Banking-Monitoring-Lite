...
exact operator wording Rule 4 mandates for each failed layer. Pure -- no I/O, no config.
...
drifted apart: the dashboard used Rule 4's locked wording while the alert email, the
thing that actually pages a human, still used pre-v3.8 phrasing. Rule 4's promise ("the
...
# neither pages (Rule 13) -- the ordering is about which status explains the platform
# verdict, not about which is more urgent to a human.
STATUS_ORDER = {"UP": 0, "DEGRADED": 1, "CONFIG_ERROR": 2, "DOWN": 3}

# Rule 4: exact operator wording, per layer. Locked by CLAUDE.md's v3.8 email-copy block --
...
def unified_verdict(main_status: str, auth_status: Optional[str]) -> str:
    ...
    didn't participate (not configured, or its own CONFIG_ERROR per Rule 10), so the main
    track alone decides."""
...
def layer_wording(fail_layer: Optional[str]) -> Optional[str]:
    """Rule 4's operator wording for the layer that failed, or None if the layer is
