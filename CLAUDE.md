...
4. Already `CONFIG_ERROR` → ignored. Only a human, or `RECOVERY_PASSES` consecutive passes **of the layer it happened on**, clears it.
...
- **`checks`** — one row per probe: `ts, ok, http_status, latency_ms, fail_reason, browser_mode, layer, burst_id, cycle_id, page_url, screenshot_path, scored`. `scored=0` marks a probe taken during the wake grace: recorded, but not counted toward DOWN. Probe-level evidence, bursts included.
